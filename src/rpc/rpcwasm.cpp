...
Value setcodewasmcontracttx(const Array& params, bool fHelp)
{
   ...
   //           << " \n";
    ...
    //     throw JSONRPCError(RPC_SCRIPT_FILEPATH_INVALID, "Lua Script file not inside /tmp/lua dir or its subdir!");
    {
        ...
        if (size <= 0 || size > kContractScriptMaxSize) { 
            throw JSONRPCError(
        ...
        if (size <= 0 || size > kContractScriptMaxSize) { // contract code size must be <= 64 KB)
            throw JSONRPCError(
                RPC_INVALID_PARAMETER,
                (size == -1) ? "File size is unknown"
...
Value callwasmcontracttx(const Array& params, bool fHelp) {
    ...
    int64_t fee    = AmountToRawValue(params[5]);
    int height     = (params.size() > 6) ? params[6].get_int() : chainActive.Height();
    if (fee == 0) {
        fee = GetTxMinFee(TxType::WASM_CONTRACT_TX, height);
    }
    ...
    CUserID sendUserId;
    CRegID sendRegId;
    sendUserId = (pCdMan->pAccountCache->GetRegId(CUserID(sender), sendRegId) && pCdMan->pAccountCache->RegIDIsMature(sendRegId))
                     ? CUserID(sendRegId)
                     : CUserID(sendPubKey);
