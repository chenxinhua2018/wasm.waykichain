...
struct COrphanBlockComparator {
    bool operator()(const COrphanBlock *pa, const COrphanBlock *pb) const {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                if (!pushed && inv.type == MSG_TX) {
                    ...
                    if (pBaseTx.get()) {
                        ...
                        }else if (WASM_CONTRACT_TX == pBaseTx->nTxType) {
                            ss << *((CWasmContractTx *)pBaseTx.get());
                        }
...
std::shared_ptr<CBaseTx> CreateNewEmptyTransaction(unsigned char uType) {
    switch (uType) {
        ...
        case WASM_CONTRACT_TX:
            return std::make_shared<CWasmContractTx>();
        default:
